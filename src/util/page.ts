...
import { Schema, Model, Document, Query as DocumentQuery, Aggregate, Types } from 'mongoose';
import { PageResult, PageOptions, Query } from '../types/db';
...
async function page(q: Query = {}, options: PageOptions): Promise<PageResult<Document>> {
  ...
  if (key) {
    if (!query._id)
      query = {
        ...query,
        ...{ _id: direction === 'previous' ? { $gt:new Types.ObjectId(key) } : { $lt: new Types.ObjectId(key) } },
      };
    else if (!query.$and)
      query = {
        ...query,
